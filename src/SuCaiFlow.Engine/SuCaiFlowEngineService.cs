...
public partial class SuCaiFlowEngineService(
    ...
    SuCaiFlowEngineTaskExecution taskExecution,
    ...
    //    try {
    //        // 标记任务为正在运行
    //        await _taskExecution.MarkTaskAsRunningAsync(taskId, cancellationToken);
    ...
    //            _taskExecution.MarkTaskAsFailed(taskId);
    //            return;
    //        }
    ...
    //        // 标记任务执行完成
    //        await _taskExecution.MarkTaskAsCompletedAsync(taskId, cancellationToken);
    ...
    //            TaskId = taskId,
    //            TaskName = task?.DisplayName ?? "Unknown",
    //            ErrorMessage = ex.Message
    //        }, cancellationToken);

    //        await _taskExecution.MarkTaskAsFailed(taskId);
    //    }
    //}

    //private async Task<List<SuCaiFlowAssetDescriptor>> ExecuteParallelCollectionAsync(SuCaiFlowTaskDescriptor task, CancellationToken cancellationToken) {
    //    var assets = new List<SuCaiFlowAssetDescriptor>();
    ...
    ///// 解析页面（带自动翻页功能）
    ///// </summary>
    //private async Task ParsePagesWithPaginationAsync(SuCaiFlowTaskDescriptor task, TaskCompletionSource<bool> parsingCompleted, CancellationToken cancellationToken) {
    ...
    ///// 解析单个页面获取资源URL
    ///// </summary>
    //private async Task<IEnumerable<string>> ParsePageAsync(SuCaiFlowTaskDescriptor task, string pageUrl, SuCaiFlowTaskConfigDescriptor? config, CancellationToken cancellationToken) {
    //    try {
    //        // 获取适合当前URL的站点采集器
    ...
    //private async Task<List<SuCaiFlowAssetDescriptor>> ExecuteCollectionAsync(SuCaiFlowTaskDescriptor task, CancellationToken cancellationToken) {
    //    var assets = new List<SuCaiFlowAssetDescriptor>();
    ...
    ///// 处理资源下载任务
    ///// </summary>
    //private async Task ProcessResourceDownloadsAsync(SuCaiFlowTaskDescriptor task, List<SuCaiFlowAssetDescriptor> assets, TaskCompletionSource<bool> parsingCompleted, CancellationToken cancellationToken) {
    //    var downloadTasks = new List<Task>();
    ...
    ///// 启动单个下载器
    ///// </summary>
    //private async Task StartDownloaderAsync(SuCaiFlowTaskDescriptor task, List<SuCaiFlowAssetDescriptor> assets, TaskCompletionSource<bool> parsingCompleted, CancellationToken cancellationToken) {
    //    // 获取任务配置
    //    var config = task.ConfigId.HasValue ?
    //        await _collectionTaskConfigRepository.GetByIdAsync(task.ConfigId.Value, cancellationToken) : null;
    //    while (true) {
    ...
    //                    existingAsset.StorageName = downloadedAsset.LocalPath;
    ...
    //private async Task ProcessAssetAsync(SemaphoreSlim semaphore, SuCaiFlowTaskDescriptor task, string assetUrl, List<SuCaiFlowAssetDescriptor> assets, CancellationToken cancellationToken) {
    ...
    //private async Task UpdateTaskProgressInDatabaseAsync(SuCaiFlowTaskDescriptor task, CancellationToken cancellationToken) {
    //    // 使用信号量确保只有一个线程可以更新数据库
    //    await _dbUpdateSemaphore.WaitAsync(cancellationToken);
    //    try {
    //        // 从数据库重新加载任务以避免并发问题
    ...
    //private async Task<SuCaiFlowAssetDescriptor?> DownloadAssetWithSiteCollectorAsync(string url, SuCaiFlowTaskConfigDescriptor? config, CancellationToken cancellationToken) {
    //    try {
    //        // 获取适合当前URL的站点采集器
    ...
    //private string PreprocessAssetUrl(string url, SuCaiFlowTaskConfigDescriptor? config) {
    //    try {
    //        // 获取适合当前URL的站点采集器
    //        var siteCollector = _siteCollectorManager.GetCollectorForUrl(url);
    //        if (siteCollector != null) {
