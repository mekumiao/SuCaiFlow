...
namespace SuCaiFlow.Core.Services
{
    public class CollectionEngineService
    {
        ...
        private readonly ICollectionTaskConfigRepository _collectionTaskConfigRepository;
        private readonly ISiteCollectorManager _siteCollectorManager;
        private readonly IEventPublisher _eventPublisher;
        private readonly ILogger<CollectionEngineService> _logger;
        private readonly HttpClient _httpClient;

        private readonly ConcurrentDictionary<Guid, int> _progressUpdates = new ConcurrentDictionary<Guid, int>();
        private readonly SemaphoreSlim _dbUpdateSemaphore = new SemaphoreSlim(1, 1); // 用于保护数据库更新的信号量
        
        // 用于存储任务进度更新的并发字典，避免在多线程中直接访问EF Core
        private readonly ConcurrentDictionary<Guid, int> _pendingProgressUpdates = new ConcurrentDictionary<Guid, int>();
        
        // 用于协调页面解析和资源下载的队列
        private readonly ConcurrentQueue<string> _resourceUrlQueue = new ConcurrentQueue<string>();
        private readonly SemaphoreSlim _queueSemaphore = new SemaphoreSlim(10, 10); // 控制资源下载的并发数

        public CollectionEngineService(
            ...
            HttpClient httpClient)
        {
            ...
            _collectionTaskConfigRepository = collectionTaskConfigRepository;
        ...
        public async Task ProcessCollectionTaskAsync(Guid taskId)
        {
            try
            {
                ...
                // 发布任务开始事件
                await _eventPublisher.PublishAsync(new SuCaiFlow.Contracts.Events.CollectionTaskStartedEvent
                {
                    TaskId = taskId,
                    TaskName = task.Name,
                    Url = task.Url
                ...
                task.Assets = assets; // 假设CollectionTask有Assets导航属性
                await _collectionTaskRepository.UpdateAsync(task);
                await _collectionTaskRepository.SaveChangesAsync();
                ...
                // 发布任务完成事件
                await _eventPublisher.PublishAsync(new SuCaiFlow.Contracts.Events.CollectionTaskCompletedEvent
                {
                    TaskId = taskId,
                    TaskName = task.Name,
            ...
            catch (Exception ex)
            {
                ...
                // 发布任务失败事件
                await _eventPublisher.PublishAsync(new SuCaiFlow.Contracts.Events.CollectionTaskFailedEvent
                {
                    TaskId = taskId,
        ...
        private async Task<List<CollectedAsset>> ExecuteParallelCollectionAsync(CollectionTask task)
        {
            ...
            try
            {
                ...
                {
                    await ParsePagesWithPaginationAsync(task, completedParsing);
                });
                
                // 启动资源下载任务
                var downloadingTask = Task.Run(async () =>
                {
                    await ProcessResourceDownloadsAsync(task, assets, completedParsing);
                });
        ...
        private async Task ParsePagesWithPaginationAsync(CollectionTask task, TaskCompletionSource<bool> parsingCompleted)
        {
            ...
            var maxParseItems = config?.MaxParseItems ?? 
                               (task.Parameters.ContainsKey("MaxParseItems") ? 
                                   int.Parse(task.Parameters["MaxParseItems"]) : 100);
            ...
            try
            {
                ...
                while (enableAutoPagination && parsedUrls.Count < maxParseItems)
                {
                    // 构造下一页URL - 优先使用配置中的模式
                    string nextPageUrl;
                    if (config != null && !string.IsNullOrEmpty(config.NextPagePattern))
                    {
                        nextPageUrl = ConstructNextPageUrlByPattern(baseUrl, currentPage + 1, config.NextPagePattern);
                    }
                    else
                    {
                        nextPageUrl = ConstructNextPageUrl(baseUrl, currentPage + 1);
                    }
        ...
        private async Task StartDownloaderAsync(CollectionTask task, List<CollectedAsset> assets, TaskCompletionSource<bool> parsingCompleted)
        {
            ...
            while (true)
            {
                ...
                // 从队列中获取资源URL
                if (_resourceUrlQueue.TryDequeue(out assetUrl))
                {
                    // 根据URL查找数据库中的素材记录
                    ...
                    if (existingAsset != null && existingAsset.CollectionTaskId == task.Id)
                    {
                        ...
                        // 发布资源下载开始事件
                        await _eventPublisher.PublishAsync(new SuCaiFlow.Contracts.Events.AssetDownloadStartedEvent
                        {
                            TaskId = task.Id,
                        ...
                        // 发布资源下载完成事件
                        await _eventPublisher.PublishAsync(new SuCaiFlow.Contracts.Events.AssetDownloadCompletedEvent
                        {
                            TaskId = task.Id,
                            AssetId = existingAsset.Id,
                            AssetUrl = absoluteUrl,
            ...
            if (_pendingProgressUpdates.TryGetValue(taskId, out int progressIncrement) && progressIncrement > 0)
            {
                ...
                try
                {
                    ...
                    if (freshTask != null)
                    {
                        ...
                        // 发布进度更新事件
                        var progressEvent = new SuCaiFlow.Contracts.Events.CollectionTaskProgressEvent
                        {
                            TaskId = freshTask.Id,
        ...
        private List<string> ExtractAssetUrls(string htmlContent, string selector)
        {
            // 简单的正则表达式匹配，实际应用中可能需要更复杂的HTML解析
            var urls = new List<string>();

            // 根据选择器类型提取URL
            if (selector.Contains("img") || selector.Contains("src"))
            {
                // 提取图片URL
                var imgRegex = new Regex(@"<img[^>]+src\s*=\s*["'](?<url>[^"']+)["']", RegexOptions.IgnoreCase);
            ...
            else if (selector.Contains("a") || selector.Contains("href"))
            {
                // 提取链接URL
                var linkRegex = new Regex(@"<a[^>]+href\s*=\s*["'](?<url>[^"']+)["']", RegexOptions.IgnoreCase);
        ...
        /// 默认的根据配置模式构造下一页URL方法
        /// </summary>
        private string ConstructNextPageUrlByPatternDefault(string baseUrl, int pageNumber, string pattern)
        {
            // 使用配置中指定的模式来构造翻页URL
        ...
        /// 默认的构造下一页URL方法
        /// </summary>
        private string ConstructNextPageUrlDefault(string baseUrl, int pageNumber)
        {
            // 这里根据不同的网站模式构造翻页URL
            // 示例：
            // 1. ?page=2 或 ?p=2
