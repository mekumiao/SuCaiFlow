...
{
    public class CollectionTaskRepository : ICollectionTaskRepository
    {
        private readonly SuCaiFlowDbContext _context;

        public CollectionTaskRepository(SuCaiFlowDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<CollectionTask?> GetByIdAsync(object id)
        {
            var dbSet = _context.CollectionTasks;
            return await dbSet
                .Include(t => t.Assets)
                .FirstOrDefaultAsync(t => t.Id == (Guid)id);
        }
